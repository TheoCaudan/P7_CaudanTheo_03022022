...
let searchResults = (e) => {
	...
	if (val.length > 2) {
		...
		for (let i = 0; i < cards.length; i++) {
			if (
				cards[i].textContent.toLowerCase().includes(val.toLowerCase())
			) {
		...
		for (let m = 0; m < appliancesFilter.length; m++) {
			for (let i = 0; i < arr.length; i++) {
				if (
					arr[i].appliance
						.toLowerCase()
						.includes(appliancesFilter[m].innerText)
				) {
...
let currentRecipeTracker = (key) => {
	...
	for (let i = 0; i < initArray.length; i++) {
		if (initArray[i].name.toLowerCase().includes(key.toLowerCase())) {
			...
			initArray[i].description.toLowerCase().includes(key.toLowerCase())
		) {
			currentRecipe.push(initArray[i]);
		} else
			for (let j = 0; j < initArray[i].ingredients.length; j++) {
				if (
					initArray[i].ingredients[j].ingredient
						.toLowerCase()
						.includes(key.toLowerCase())
				) {
					currentRecipe.push(initArray[i]);
